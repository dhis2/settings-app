...
function configI18n(userSettings) {
    ...
    if (uiLocale !== 'en') {
        config.i18n.sources.add(
            `i18n/module/i18n_module_${uiLocale}.properties`
        )
    }
...
getManifest('manifest.webapp')
    ...
    .then(
        d2 => {
            ...
            const baseUrl = apiBaseUrl.substr(
                0,
                apiBaseUrl.lastIndexOf('/api/')
            )
            Promise.all([
                d2.models.indicatorGroup.list({
                    paging: false,
                    fields: 'id,displayName',
                    order: 'displayName:asc',
                }),
                d2.models.dataElementGroup.list({
                    paging: false,
                    fields: 'id,displayName',
                    order: 'displayName:asc',
                }),
                d2.models.userGroup.list({
                    paging: false,
                    fields: 'id,displayName',
                    order: 'displayName:asc',
                }),
                ...
                // Flags
                const flags = (results[7] || []).map(flag => ({
                    id: flag.key,
                ...
                // Stylesheets
                const styles = (results[8] || []).map(style => ({
                    id: style.path,
                    displayName: style.name,
                }))

                // Locales
                const locales = (results[9] || []).map(locale => ({
                    id: locale.locale,
